jQuery(document).ready(function ($) {
	...
	$(document.body).on('click.eddAddToCart', '.edd-add-to-cart', function (e) {
		...
		let form = $this.parents('form').last();
		...
		let item_data = JSON.parse(form.find('.gtmkit_product_data').val());
		...
		if (variable_price === 'yes') {
			...
			form.find('.edd_price_option_' + download + ':checked', form).each(function (index) {
				...
				if (price_mode === 'multi') {
					quantity = parseInt(form.find('.edd-item-quantity' + '[name="edd_download_quantity_' + $(this).val() + '"]').val());
